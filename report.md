...
Пропустим часть с тренировкой и сразу приступим к заданию. Нам нужно сделать такой файл `bash`, который будет принимать на входе ipv4 адрес (примем условность, что он будет верным всегда, то есть каждый октет не больше 255 и всего и 4 через точку)
